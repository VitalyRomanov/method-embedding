...
def train_step_finetune(epoch_frac, model, optimizer, token_ids, prefix, suffix, graph_ids, labels, lengths,
                   class_weights=None, scorer=None):
    with tf.GradientTape() as tape:
        logits = model(token_ids, prefix, suffix, graph_ids, training=True)
        loss = model.loss(logits, labels, lengths, class_weights=class_weights)
    ...
    try:
        ...
        for e in range(epochs):
            ...
            for ind, batch in enumerate(train_batches):
                ...
                loss, p, r, f1 = train_step_finetune(e/epochs, model=model, optimizer=optimizer, token_ids=batch['tok_ids'],
