...
class ElementEmbedderBase:
    def __init__(self, elements, compact_dst=True):
        # labels must be compacted
        self.init(elements, compact_dst)

    def compact_dst(self, elements, compact_dst):
        if compact_dst:
            elem2id, self.inverse_dst_map = compact_property(elements['dst'], return_order=True)
            compacted = elements['dst'].apply(lambda x: elem2id.get(x, -1))
        else:
            compacted = elements['dst']
        return compacted
    ...
    def init(self, elements, compact_dst):
        ...
        for src, dst in zip(elements["src"], compacted):
            self.element_lookup[src].append(dst)

        self.num_unique_targets = elements["dst"].nunique()

        self.init_neg_sample(elements)

    # def preprocess_element_data(self, element_data):
    #     """
    ...
    def init_neg_sample(self, elements, skipgram_sampling_power=0.75):
        # compute distribution of dst elements
        counts = elements['dst'].value_counts(normalize=True)
