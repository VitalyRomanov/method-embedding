...
def main():
    ...
    id_offset = read_nodes(working_directory)["id"].max() + 1
    bodies = read_processed_bodies(working_directory)[['id', 'body']].dropna(axis=0)
    ...
    if not func_var_pairs:
        pairs = pd.DataFrame(columns=['src', 'dst'])
    else:
