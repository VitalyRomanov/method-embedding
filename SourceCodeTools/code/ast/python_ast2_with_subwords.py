from SourceCodeTools.code.ast.python_ast2 import PythonNodeEdgeDefinitions, PythonCodeExamplesForNodes, \
    PythonAstGraphBuilder, GEdge
from SourceCodeTools.code.data.ast_graph.draw_graph import visualize
from SourceCodeTools.nlp import create_tokenizer


class PythonAstWithSubwordsEdgeDefinitions(PythonNodeEdgeDefinitions):

    extra_node_types = PythonNodeEdgeDefinitions.extra_node_types | {
...
def make_python_ast_graph_with_subwords(
        source_code, add_reverse_edges=False, save_node_strings=False
):
    g = PythonAstWithSubwordsGraphBuilder(
        source_code, PythonAstWithSubwordsEdgeDefinitions, add_reverse_edges=add_reverse_edges,
...
if __name__ == "__main__":
    ...
    nodes, edges = make_python_ast_graph_with_subwords(c.lstrip(), add_reverse_edges=True)
    visualize(nodes, edges, "/Users/LTV/1.png")