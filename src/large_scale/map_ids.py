...
edges[['type','source_node_id','target_node_id']].to_csv(os.path.join(os.path.dirname(sys.argv[2]), sys.argv[5]), index=False, header=False)

bodies['id'] = bodies['id'].apply(lambda x: id_map[x])
bodies.to_csv(os.path.join(os.path.dirname(sys.argv[2]), sys.argv[6]), index=False, header=False)