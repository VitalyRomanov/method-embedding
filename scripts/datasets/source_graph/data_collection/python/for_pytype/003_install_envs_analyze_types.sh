...
#  echo $repo/bin/python
  ...
  if $act_env; then
    ...
    if [ ! -f "$repo/packages.txt" ]; then
      ...
      ls $repo/lib/python3.7/site-packages > $repo/env_site_packages.txt
      pip install pytype==2023.2.17 pyre-check==0.9.18 > $repo/piplog_install_analyzers.log
      pip freeze > $repo/packages_with_analyzers.txt
#    else
#      cat $repo/packages.txt | xargs pip install > $repo/piplogsecondary.log
    fi

    if [ ! -f "$repo/type_annotations.json" ]; then
      python ../strip_annotations_and_defaults.py $repo $repo/lib/python3.7/site-packages type_annotations.json
    fi
