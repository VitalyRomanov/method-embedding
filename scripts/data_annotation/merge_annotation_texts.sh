...
for dir in $(ls $ENVS_DIR); do
  if [ -d $ENVS_DIR/$dir ]; then
    ...
    if [ -f $ENVS_DIR/$dir/normalized_sourcetrail_nodes.csv ]; then
      ...
      sourcecodetools-extract-type-annotations.py $ENVS_DIR/$dir/source-graph-bodies.csv $ENVS_DIR/functions_with_annotations.jsonl $ENVS_DIR/$dir/local2global.csv $ENVS_DIR/common_nodes_with_ast.csv $ENVS_DIR/$dir/nodes_with_ast.csv
    fi
  fi
done
