...
#     edges = edges.copy()
#
#     node_id_map = compact_property(nodes['id'])
#     nodes['new_id'] = nodes['id'].apply(lambda x : node_id_map[x])
#     # nodes['new_id'] = list(range(nodes.shape[0]))
#
#     import pandas
#     assert any(pandas.isna(nodes['new_id'])) == False
#
...
#
#     # type_map = compact_property(nodes['label'])
#     uniq_node_types = np.sort(nodes['label'].unique())
#     type_map = dict(zip(uniq_node_types, range(len(uniq_node_types))))
